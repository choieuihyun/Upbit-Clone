...
class CoinInfoCoinTrendHighestVolumePowerRateAdapter :
    ListAdapter<CoinInfoCoinTrendVolumePowerRateModel, CoinInfoCoinTrendHighestVolumeRateViewHolder>(
        diffCallback) {
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): CoinInfoCoinTrendHighestVolumeRateViewHolder {

        return CoinInfoCoinTrendHighestVolumeRateViewHolder(
            FragmentCoinInfoCointrendVolumepowerRateListItemBinding.inflate(LayoutInflater.from(parent.context),parent, false),coinTickersMap)


    }

    override fun onBindViewHolder(
        holder: CoinInfoCoinTrendHighestVolumeRateViewHolder,
        position: Int
    ) {

        val coinVolumeData = currentList[position]
        holder.bind(coinVolumeData)

    }

    companion object {

        private val diffCallback = object : DiffUtil.ItemCallback<CoinInfoCoinTrendVolumePowerRateModel>() {

            override fun areItemsTheSame(oldItem: CoinInfoCoinTrendVolumePowerRateModel, newItem: CoinInfoCoinTrendVolumePowerRateModel): Boolean {
                return oldItem.hashCode() == newItem.hashCode()
            }

            override fun areContentsTheSame(oldItem: CoinInfoCoinTrendVolumePowerRateModel, newItem: CoinInfoCoinTrendVolumePowerRateModel): Boolean {
